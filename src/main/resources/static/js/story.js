...
function storyLoad() {
    $.ajax({
        ...
        console.log("스토리 불러오기 성공", res);
        res.data.forEach((i)=>{
            let item = getStoryItem(i);
            $("#storyList").append(item);
        });

    }).fail(error=>{
        console.log("스토리 불러오기 실패", error);
    });
...
function getStoryItem(image) {
 let item = `<div class="story-list__item">
                <div class="sl__item__header">
                    <div>
                        <img class="profile-image" src="/upload/${image.user.userImageUrl}"
...
// (3) 좋아요(fa-heart fas active), 안좋아요(fa-heart far)
...
function deleteComment(commentId) {
    $.ajax({
        type:"delete",
        url:`/api/image/comment/${commentId}`,
        data:"json"
    }).done(res=>{
